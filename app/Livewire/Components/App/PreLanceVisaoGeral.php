...
class PreLanceVisaoGeral extends Component
{
    ...
    public function mount(Leilao $leilao)
    {
        ...
        //     //->where('lance.tipo', TipoLanceEnum::PRELANCE) // Apenas lances do tipo "PRELANCE"
        //     ->whereIn('lance.uuid', function ($query) {
        //         $query->select(DB::raw('MAX(lance.uuid)'))
        //             ->from('lance')
        //             ->join('lote', 'lance.lote_uuid', '=', 'lote.uuid')
        //             //->where('lance.tipo', TipoLanceEnum::PRELANCE) // Apenas lances do tipo "PRELANCE" na subquery
        //             ->groupBy('lote.uuid');
        //     })
        ...
        //             ->select('l1.lote_uuid', DB::raw('MAX(l1.valor) as ultimo_lance_uuid'))
        //             ->join('lote', 'l1.lote_uuid', '=', 'lote.uuid')
        //             ->groupBy('l1.lote_uuid'),
        //         'subquery_lances',
        //         'subquery_lances.ultimo_lance_uuid',
        //         '=',
        //         'lance.uuid'
        //     )
        //     ->joinSub(
        ...
        ->joinSub(
            DB::table('lance as l1')
                ...
                ->join('lote', 'l1.lote_uuid', '=', 'lote.uuid')
                // ->where('l1.tipo', 'PRELANCE') // Apenas lances do tipo "PRELANCE"
                ->groupBy('l1.lote_uuid'),
            'subquery_lances',
            function ($join) {
